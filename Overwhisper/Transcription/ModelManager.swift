...
class ModelManager: ObservableObject {
  ...
  init(appState: AppState) {
    ...
    self.modelsDirectory = appSupport.appendingPathComponent(
      "Overwhisper/Models", isDirectory: true)
  ...
  func scanForModels() {
    ...
    let possiblePaths = [
      ...
      appSupport.appendingPathComponent(
        "MacWhisper/models/whisperkit/models/argmaxinc/whisperkit-coreml"),
    ...
    // Check for Parakeet models in FluidAudio cache
    scanForParakeetModels(homeDir: homeDir, foundModels: &foundModels)
    ...
    case .parakeet:
      appState.isModelDownloaded = foundModels.contains(appState.parakeetModel.rawValue)
    case .openAI:
      appState.isModelDownloaded = true
    }
  }

  private func scanForParakeetModels(homeDir: URL, foundModels: inout Set<String>) {
    // FluidAudio stores models in ~/.cache/fluidaudio/
    let fluidaudioCache = homeDir.appendingPathComponent(".cache/fluidaudio/Models")

    guard
      let contents = try? FileManager.default.contentsOfDirectory(
        at: fluidaudioCache, includingPropertiesForKeys: nil)
    else {
      return
    }
    ...
    for url in contents where url.hasDirectoryPath {
      ...
      if name.contains("parakeet-tdt") {
        // Detect v2 vs v3 based on directory name
        if name.contains("-v2") {
          foundModels.insert(ParakeetModelType.v2English.rawValue)
        } else if name.contains("-v3") {
          foundModels.insert(ParakeetModelType.v3Multilingual.rawValue)
        }
      }
    }
  }
    ...
    for url in contents where url.hasDirectoryPath {
      ...
      // Check for huggingface hub cache format
        ...
        if let snapshots = try? FileManager.default.contentsOfDirectory(
          at: snapshotsPath, includingPropertiesForKeys: nil)
        {
          for snapshot in snapshots where snapshot.hasDirectoryPath {
            if let modelDirs = try? FileManager.default.contentsOfDirectory(
  ...
  func deleteModel(_ modelName: String) throws {
    ...
    for basePath in searchPaths {
      ...
      for url in contents where url.hasDirectoryPath {
        ...
        if name == "openai_whisper-\(modelName)" || name.hasPrefix("openai_whisper-\(modelName)-v")
          || name == modelName
        {
    ...
    let hubCachePath = homeDir.appendingPathComponent(
      ".cache/huggingface/hub/models--argmaxinc--whisperkit-coreml/snapshots")
    if let snapshots = try? fileManager.contentsOfDirectory(
      at: hubCachePath, includingPropertiesForKeys: nil)
    {
      for snapshot in snapshots where snapshot.hasDirectoryPath {
        if let modelDirs = try? fileManager.contentsOfDirectory(
          at: snapshot, includingPropertiesForKeys: nil)
        {
          for modelDir in modelDirs where modelDir.hasDirectoryPath {
  ...
  func findModelFolder(for modelName: String) -> String? {
    ...
    if let repoDirs = try? FileManager.default.contentsOfDirectory(
      at: hubCachePath, includingPropertiesForKeys: nil)
    {
  ...
  private func validateModelChecksum(at modelFolder: URL) throws {
    ...
    guard
      ...
      throw ModelDownloadError.validationFailed(
        "Unable to enumerate model files for checksum validation.")
    }
