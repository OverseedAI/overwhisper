...
enum ParakeetModelType: String, CaseIterable, Identifiable {
  ...
  var description: String {
    switch self {
    case .v2English: return "English only, highest accuracy"
...
struct HotkeyConfig: Codable, Equatable {
  ...
  static let defaultPushToTalk = HotkeyConfig(
    keyCode: UInt32(kVK_Space), modifiers: UInt32(optionKey | shiftKey))
...
class AppState: ObservableObject {
  ...
  @Published var muteSystemAudioWhileRecording: Bool {
    didSet {
      UserDefaults.standard.set(
        muteSystemAudioWhileRecording, forKey: "muteSystemAudioWhileRecording")
    }
  }
  ...
  @Published var recordingDurationLimitEnabled: Bool {
    didSet {
      UserDefaults.standard.set(
        recordingDurationLimitEnabled, forKey: "recordingDurationLimitEnabled")
    }
  }
  @Published var recordingDurationLimitSeconds: Int {
    didSet {
      UserDefaults.standard.set(
        recordingDurationLimitSeconds, forKey: "recordingDurationLimitSeconds")
    }
  }
  ...
  init() {
    ...
    let modeStr =
      UserDefaults.standard.string(forKey: "recordingMode") ?? RecordingMode.toggle.rawValue
    self.recordingMode = RecordingMode(rawValue: modeStr) ?? .toggle

    let posStr =
      UserDefaults.standard.string(forKey: "overlayPosition")
      ?? OverlayPosition.bottomRight.rawValue
    self.overlayPosition = OverlayPosition(rawValue: posStr) ?? .bottomRight

    let engineStr =
      UserDefaults.standard.string(forKey: "transcriptionEngine")
    ...
    let modelStr =
      UserDefaults.standard.string(forKey: "whisperModel") ?? WhisperModel.smallEn.rawValue
    self.whisperModel = WhisperModel(rawValue: modelStr) ?? .smallEn

    let parakeetModelStr =
      UserDefaults.standard.string(forKey: "parakeetModel")
      ?? ParakeetModelType.v3Multilingual.rawValue
    ...
    self.playSoundOnCompletion =
      UserDefaults.standard.object(forKey: "playSoundOnCompletion") as? Bool ?? true
    self.playSoundOnStart = UserDefaults.standard.bool(forKey: "playSoundOnStart")
    self.showNotificationOnError =
      UserDefaults.standard.object(forKey: "showNotificationOnError") as? Bool ?? true
    self.muteSystemAudioWhileRecording = UserDefaults.standard.bool(
      forKey: "muteSystemAudioWhileRecording")
    self.selectedInputDeviceUID =
      UserDefaults.standard.string(forKey: "selectedInputDeviceUID") ?? ""
    self.recordingDurationLimitEnabled = UserDefaults.standard.bool(
      forKey: "recordingDurationLimitEnabled")
  ...
  func addDebugLog(
    _ message: String, level: DebugLogEntry.DebugLogLevel = .info, source: String = "App"
  ) {
